...
def make_session() -> requests.Session:
    ...
    retries = Retry(
        ...
        connect=5,
        read=5,
        backoff_factor=1.0,  # Wait longer between retries
...
def submit_query(session: requests.Session, dataset_id: str, limit: int) -> str:
    ...
    # The API returns the task ID as a quoted string in the response body
...
def fetch_query_result(session: requests.Session, task_id: str) -> List[Dict]:
    ...
    while time.time() - start_time < REACTION_TIMEOUT_S:
        ...
        # Status 202 (Accepted) means the job is still running
        elif resp.status_code == 202:
            time.sleep(2.0)
            continue
        
        # Status 400 (Bad Request) sometimes means the job failed or is still pending
        elif resp.status_code == 400 and "not ready" in resp.text.lower():
            time.sleep(2.0)
            continue
        
        resp.raise_for_status() # Handle other errors (e.g., 404)
...
def extract_reaction_data(rxn, dataset_id: str) -> Dict[str, Any]:
    ...
    try:
        # Check if the reaction succeeded
        outcome_successful = False
        if rxn.outcomes and rxn.outcomes[0].reaction_product:
            outcome_successful = True # Simple check for product existence

        # Iterate over components grouped by their input key (e.g., 'solvent')
        for input_key, reaction_input in rxn.inputs.items():
            ...
            for component in reaction_input.components:
                ...
                # Get the human-readable role name (e.g., 'REACTANT', 'SOLVENT')
                try:
                    from ord_schema.proto import reaction_pb2
                ...
                # Assign to one of the CORE_CATEGORIES based on a simple name check
                assigned = False
                ...
                # If not a core category, add it under its input_key
                if not assigned:
...
def scrape_ord_advanced(
    ...
    for ds in datasets:
        ...
        # Politely rate limit between datasets
        time.sleep(1.0) 
...
Examples:
  ...
  python ord_advanced_scraper.py --dataset_ids ord_dataset-3b7692e9d29b43179261358b13997fef --limit 0
    ...
    if args.max_datasets == 0 and not ds_ids:
        ...
        args.max_datasets = None # Set to None to process all
